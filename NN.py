...
class NN(nn.Module):
    ...
    def forward(self, tensor):
        ...
        if self.normalizer != None:
            tensor = self.normalizer.Normalize(tensor).float()
        ...
        if self.normalizer != None:
            tensor = self.normalizer.Denormalize(tensor).float()
        return tensor
...
class Main_Network(nn.Module):
    ...
    def forward(self, x, z):
        ...
        if self.normalizer != None:
            norm_x_hat = self.normalizer.Normalize(output_xzx).float()
            norm_z_hat = self.normalizer.Normalize(output_xz).float()
        else:
